...
list_apps() {
    ...
    local flatpak_system="/var/lib/flatpak/exports/share/applications"
    local flatpak_user="$HOME/.local/share/flatpak/exports/share/applications"
    
    find "$apps_dir" "$local_apps_dir" "$flatpak_system" "$flatpak_user" -name "*.desktop" 2>/dev/null | \
...
# Establecer navegador predeterminado
set_browser() {
    browsers=$(list_apps | grep -iE "firefox|chrome|chromium|brave|vivaldi|opera|qutebrowser|falkon|zen|browser")
